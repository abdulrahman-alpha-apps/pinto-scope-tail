...
Source: `workflows/remote/dev_4/subagents/Expense agent V3/tools/creat vendor.json`

Purpose:
Creates a vendor for the OCR-driven expense path.

Technical flow:
This is the shorter vendor-creation variant used by the automated flow. It checks for existing matches, iterates over candidate results, creates a vendor if needed, sends user notifications, and stores the selected vendor identity back into shared state.
...
flowchart TD
  ...
  Upsert_row_s_4_14 -->|main| Edit_Fields1_3
```
